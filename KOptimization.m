function [Population, Model, A1, THETA,V,Rate] = KOptimization(Model, PopDec, V, V0, alpha, wmax, mu, NI, THETA, A1, TransFlag,Gen,Rate)  
    global Global;
    w        = 1;
    A1Obj    = objs(A1);
    PopDec   = decs(A1);
    ...
    if Global.t > Global.SerNum && Rate<=0
        [TransDec,TransObj,TransMSE,NewCenter] = PPSK(PopDec,Model);
        if Gen == 1
    ...
    if Global.t > Global.SerNum && CalRateFlag
    ...
    if Global.t>Global.Rs
        A1 = KHisAddArchieve(A1, TransFlag);
    end
    A1 = KHisUpdataArchive(A1,New,V,mu,NI, TransFlag);  
    if Global.Evaluated >= Global.t * Global.T
        Global.MultiR(Global.t).OptimalPop = Population;
    end
    %% ReTrain
    for j = 1 : Global.M
